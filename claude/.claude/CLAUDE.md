...
- **When creating PRs (via `/pr`)**, always rebase from main/master first and resolve all conflicts (if any) before creating the PR. The full PR workflow (review bot loop, CI checks, merge) is defined in the `/pr` command — follow it exactly.
